...
public class Minecraft implements IThreadListener
{
    ...
    public Minecraft(GameConfiguration gameConfig)
    {
        ...
        this.asyncScreenshot = gameConfig.isAsyncScreenshot();
        ...
        if (i != 0 && !Keyboard.isRepeatEvent())
        {
            if (!(this.currentScreen instanceof GuiControls) || ((GuiControls)this.currentScreen).time <= getSystemTime() - 20L)
            {
                if (Keyboard.getEventKeyState())
                {
                    ...
                    {
                        IChatComponent text = ScreenShotHelper.saveScreenshot(this.mcDataDir, this.displayWidth, this.displayHeight, this.framebufferMc);
                        if (!text.getUnformattedText().equalsIgnoreCase("null")) {
